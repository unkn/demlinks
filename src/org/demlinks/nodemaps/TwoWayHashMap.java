...
public class TwoWayHashMap<Key, Value> {
	...
	public boolean putKeyValue( Key _k, Value _v ) {
		...
		Value prevFwdVal = this.forward.put( _k, _v );
		...
		Key prevBackwdKey = this.backward.put( _v, _k );
	...
	public Value removeKey( Key _k ) {
		...
		Debug.nullException( _k );
		Value deleted = this.forward.remove( _k );
		Key tmpk = this.backward.remove( deleted );
		if ( tmpk != _k ) {
			throw new AssertionError(
					"impartial removal, how?! and this is weird!" );
		}
		return deleted;
	}
	...
	public Key removeValue( Value _v ) {
		...
		Debug.nullException( _v );
		Key _k = this.getKey( _v );
		if ( _v != this.removeKey( _k ) ) {
			throw new AssertionError(
					"we removed the value of another key, this means 2 keys had same value in this 1 to 1 MAP" );
		}
		return _k;
	}
