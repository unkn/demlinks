...
public class ListOrderedOfElementCapsules {
	 ...
	 */
	protected ListOrderedOfElementCapsules( Level040_DMLEnvironment passedEnv, DomainSet passedDS ) {

		RunTime.assumedNotNull( passedEnv, passedDS );
	...
	private final static void registerInstance( Level040_DMLEnvironment env, DomainSet domainSet,
			ListOrderedOfElementCapsules newOne ) {

		RunTime.assumedNotNull( env, domainSet, newOne );
		RunTime.assumedFalse( allListOOECInstances.ensure( env, domainSet, newOne ) );
	}
	
	private final static ListOrderedOfElementCapsules getInstance( Level040_DMLEnvironment env, DomainSet domainSet ) {

		RunTime.assumedNotNull( env, domainSet );
		return allListOOECInstances.get( env, domainSet );
	}
	 ...
	 * @return
	 */
	public static ListOrderedOfElementCapsules getListOOEC( Level040_DMLEnvironment passedEnv, Symbol existingSymbol ) {
		...
		ListOrderedOfElementCapsules existingList = getInstance( passedEnv, existingDS );
		if ( null == existingList ) {
			internal_setAsListOrderedOfECs( passedEnv, existingDS.getAsSymbol() );
			existingList = new ListOrderedOfElementCapsules( passedEnv, existingDS );
			existingList.assumedValid();
			registerInstance( passedEnv, existingDS, existingList );
		} else {
			existingList.assumedValid();
		}
		return existingList;
	...
	private static void internal_setAsListOrderedOfECs( Level040_DMLEnvironment passedEnv, Symbol passedSelf ) {
	...
	protected boolean isItself() {
		...
		RunTime.assumedNotNull( self, env );
		RunTime.assumedTrue( env.isInited() );
		return ListOrderedOfElementCapsules.isListOrderedOfECs( env, self.getAsSymbol() );
	}
	
	public static boolean isListOrderedOfECs( Level040_DMLEnvironment passedEnv, Symbol passedSelf ) {
	...
	public void assumedValid() {
		...
		if ( null != first ) {
			RunTime.assumedTrue( env.isVector( self.getAsSymbol(), first.getAsSymbol() ) );
		...
		if ( null != last ) {
			RunTime.assumedTrue( env.isVector( self.getAsSymbol(), last.getAsSymbol() ) );
	...
	private void internal_dmlRegisterNewHeadOrTail( Position pos, ElementCapsule candidate ) {

		RunTime.assumedNotNull( pos, candidate );
		candidate.assumedIsValidCapsule();
		this.internal_dmlRegisterNewHeadOrTailAllowNull( pos, candidate );
	}
	 ...
	 * @param candidate
	 */
	private ElementCapsule internal_dmlRegisterNewHeadOrTailAllowNull( Position pos, ElementCapsule candidate ) {
	 ...
	 * @param posElement
	 */
	public void add_ElementCapsule( ElementCapsule theNew, Position pos, ElementCapsule posElement ) {
		...
		case AFTER:
			...
			if ( null == oldOne ) {
				...
				// it's not the last
				// oldnext.prev=theNew
				oldOne.setSideCapsule( Position.opposite( pos ), theNew );
			}
			

			// oldOne can be null here, from above, depending on case; and is ok
			theNew.setSideCapsule( pos, oldOne );// new.next=oldnext
			
			posElement.setSideCapsule( pos, theNew );// next=theNew
			
			// new.prev=posElement
			theNew.setSideCapsule( Position.opposite( pos ), posElement );
			if ( theNew.getSideCapsule( pos ) == null ) {
				// no prev, then make it FIRST
				this.internal_dmlRegisterNewHeadOrTail( Position.getAsEdge( pos ), theNew );
			}
			break;
		default:
			RunTime.badCall( "bad position for this method" );
	...
	public void add_ElementCapsule( Position pos, ElementCapsule theNew ) {
		...
		case LAST:
			...
			if ( null != theOld ) {
				...
				theNew.setSideCapsule( Position.opposite( Position.getAsNear( pos ) ), theOld );
				// oldfirst.prev=newfirst
				theOld.setSideCapsule( Position.getAsNear( pos ), theNew );
				// setFirst=newfirst
				theNew.assumedIsValidCapsule();
			} else {
				// no first then no last
				// set as Tail aka last
				this.internal_dmlRegisterNewHeadOrTail( Position.opposite( pos ), theNew );
			}
	...
	public ElementCapsule get_ElementCapsule( Position pos ) {
		...
		Symbol x = env.findCommonTerminalForInitials( sym.getAsSymbol(), self.getAsSymbol() );
		if ( null != x ) {// found one
			// existing ElementCapsule, wrap it in ElementCapsule type
			ElementCapsule ec = ElementCapsule.getExistingElementCapsule( env, x );
			return ec;
		}
	...
	public ElementCapsule removeEC( Position pos, ElementCapsule posEC ) {
		...
		ElementCapsule toRemove = this.get_ElementCapsule( pos, posEC );
		if ( null != toRemove ) {
			RunTime.assumedTrue( this.remove( toRemove ) );
	...
	public boolean remove( ElementCapsule whichEC ) {
		...
		if ( ( null == prev ) && ( null == next ) ) {
			...
			RunTime.assumedTrue( this.internal_dmlRegisterNewHeadOrTailAllowNull( Position.FIRST, null ) == whichEC );
			RunTime.assumedTrue( this.internal_dmlRegisterNewHeadOrTailAllowNull( Position.LAST, null ) == whichEC );
		} else {
			if ( null == prev ) {
				...
				RunTime.assumedTrue( this.internal_dmlRegisterNewHeadOrTailAllowNull( Position.FIRST, next ) == whichEC );
			} else {
				prev.setSideCapsule( Position.AFTER, next );// next can be null
			}
			if ( null == next ) {
				...
				RunTime.assumedTrue( this.internal_dmlRegisterNewHeadOrTailAllowNull( Position.LAST, prev ) == whichEC );
			} else {
				next.setSideCapsule( Position.BEFORE, prev );// prev an be null
			}
		}// if
