...
public class ElementCapsule
{
	 ...
	 * 
	 */
	private ElementCapsule(
			Level040_DMLEnvironment passedEnv,
			SetOfTerminalSymbols existingSelfAsSet,
			DomainPointer existingRef2Prev,
			Pointer existingRef2Element,
			DomainPointer existingRef2Next )
	{
								...
								ElementCapsule newOne )
	{
		...
		RunTime.assumedNotNull(
								...
								self,
								newOne );
		RunTime.assumedFalse( allElementCapsuleInstances.ensure(
																	passedEnv,
																	self,
																	newOne ) );
	}
							...
							SetOfTerminalSymbols self )
	{
		...
		RunTime.assumedNotNull(
								passedEnv,
								self );
		return allElementCapsuleInstances.get(
												passedEnv,
												self );
	}
										...
										Symbol existingSymbol )
	{
		...
		ElementCapsule existingEC = getInstance(
													passedEnv,
													existingSet );
		if ( null == existingEC )
		{
			...
			Symbol ref2Prev = passedEnv.findCommonTerminalForInitials(
																		passedEnv.allPrevElementCapsules_Set
																				.getAsSymbol(),
																		existingSymbol );
			RunTime.assumedNotNull( ref2Prev );
			DomainPointer existingRef2Prev = passedEnv.getExistingDomainPointer(
																					ref2Prev,
																					passedEnv.allElementCapsules_Set
																							.getAsSymbol(),
																					true );
			// 2
			Symbol ref2Next = passedEnv.findCommonTerminalForInitials(
																		passedEnv.allNextElementCapsules_Set
																				.getAsSymbol(),
																		existingSymbol );
			RunTime.assumedNotNull( ref2Next );
			DomainPointer existingRef2Next = passedEnv.getExistingDomainPointer(
																					ref2Next,
																					passedEnv.allElementCapsules_Set
																							.getAsSymbol(),
																					true );
			// 3
			Symbol ref2Elem = passedEnv
					.findCommonTerminalForInitials(
													passedEnv.allRef2ElementsInEC_Set.getAsSymbol(),
													existingSymbol );
			RunTime.assumedNotNull( ref2Elem );
			Pointer existingRef2Element = passedEnv.getExistingPointer(
																		ref2Elem,
																		true );
			
			existingEC = new ElementCapsule(
												passedEnv,
												existingSet,
												existingRef2Prev,
												existingRef2Element,
										...
										Symbol existingSymbol )
	{
		...
		// allECs->self
		internal_setAsElementCapsule(
										passedEnv,
										existingSymbol );
		ElementCapsule existingEC = new ElementCapsule(
														passedEnv,
														existingSet,
														newRef2Prev,
														newRef2Element,
											...
											Symbol passedSelf )
	{
		...
		RunTime.assumedTrue( isElementCapsule(
												passedEnv,
												passedSelf ) );
	}
			...
			assumedIsValidCapsule()
	{
		...
		long size = selfAsSet.size();// env.countTerminals( selfAsSymbol );
							...
							Position pos )
	{
		...
		if ( null != theEC )
		{
			return ElementCapsule.getExistingElementCapsule(
																env,
																theEC );
		}
		else
