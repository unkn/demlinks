...
public class TwoWayHashMap<Key, Value> {
	...
	public boolean putKeyValue( final Key _k, final Value _v ) {
		assert Q.nn( _k );
		assert Q.nn( _v );
		final Value prevFwdVal = forward.put( _k, _v );
		...
		final Key prevBackwdKey = backward.put( _v, _k );
	...
	public Value removeKey( final Key _k ) {
		assert Q.nn( _k );
		final Value deleted = forward.remove( _k );
		final Key tmpk = backward.remove( deleted );
		if ( tmpk != _k ) {
