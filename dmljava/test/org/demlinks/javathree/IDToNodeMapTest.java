...
public class IDToNodeMapTest {
	...
	public void testGetNode() throws Exception {
		assertTrue(imap.put(a, nodea));//first time
		showAllKeys("1");
		assertTrue(imap.getNode(a) == nodea);
		assertFalse(imap.put(b, nodeb));//replaced
		showAllKeys("2");
		...
		assertTrue(imap.getID(nodeb).equals(a));
		assertTrue(imap.getID(nodeb).equals(b));
		assertTrue(imap.getID(nodeb) == b);
		
		assertFalse(imap.put(c, nodea));//2nd time
		assertFalse(imap.put(c, nodeb));//3rd time
		assertTrue(imap.size() == 1);
		assertFalse(imap.put(g, nodeb));//nodeb already exists
		assertTrue(imap.size() == 1);
		assertFalse(imap.getNode(g) == null);
		assertTrue( nodeb == imap.removeID(g) );
		assertTrue(imap.getNode(g) == null);
		assertTrue(imap.put(g, nodea));//new
		assertTrue(imap.size() == 1);
		showAllKeys("3");
		...
		assertTrue(imap.getID(nodea).equals(b));
		assertTrue(imap.getID(nodea).equals(c));
		assertTrue(imap.getID(nodea).equals(d));
		assertTrue(imap.getID(nodea).equals(e));
		assertTrue(imap.getID(nodea).equals(f));
		assertTrue(imap.size() == 1);
		assertTrue(imap.getNode(a) == nodea);
