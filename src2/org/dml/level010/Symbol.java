...
public class Symbol
{
	...
	// FIXME: what happens when bdb gets deinited? and when another one gets inited that may have same address
	// supposedly
	...
	/**
	 * call this before deInit-ing BDB
	 * 
	 * @param bdb
	 */
	public static
			void
			removeCachedSymbolsFromThisBDBL1(
												Level1_Storage_BerkeleyDB bdb )
	{
		RunTime.assumedNotNull( bdb );
		RunTime.assumedTrue( bdb.isInitedSuccessfully() );
		
		TwoWayHashMap<Symbol, TheStoredSymbol> temp2WayHashMap = all_Symbols_from_BDBStorage.get( bdb );
